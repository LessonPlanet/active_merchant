...
class RemoteLitleCertification < Test::Unit::TestCase
  ...
  def test57_58
    ...
    # authorize
    assert response = @gateway.authorize(15000, card_token, options)
  ...
  def test59
    card_token = LitleCardToken.new(:token => '1712990000040196',
                                    :month => '11',
                                    :year  => '2014')
    options = {
        :order_id => '59'
  ...
  def test60
    card_token = LitleCardToken.new(:token => '171299999999999',
                                    :month => '11',
                                    :year  => '2014')
    options = {
        :order_id => '60'
  ...
  def test_authorize_and_purchase_and_credit_with_card_token
    options = {
        :order_id => '12345678',
        :billing_address => {
            :name => 'John Smith',
    ...
    assert purchase_response = @gateway.purchase(100, card_token, options.merge({order_id: '12345679'}))
    assert_success purchase_response
    assert_equal 'Approved', purchase_response.message

    # credit
    assert credit_response = @gateway.credit(500, card_token, options.merge({order_id: '12345670'}))
  ...
  def auth_assertions(amount, card, options, assertions)
    ...
    # 1A: capture
    assert response = @gateway.capture(amount, response.authorization, {order_id => "#{order_id}A"})
    assert_equal 'Approved', response.message

    # 1B: credit
    assert response = @gateway.credit(amount, response.authorization, {order_id => "#{order_id}B"})
    assert_equal 'Approved', response.message

    # 1C: void
    assert response = @gateway.void(response.authorization, {order_id => "#{order_id}C"})
    assert_equal 'Approved', response.message
  end
