...
class LitleTest < Test::Unit::TestCase
  ...
  def test_build_purchase_request_with_card_token
    # define all inputs
    money      = 1000
    ...
    hash_from_gateway = @gateway.send(:build_purchase_request, money, card_token, {})
  ...
  def test_build_authorize_request_with_card_token
    # define all inputs
    money      = 1000
    token      = '1234567890123456'
    month      = 9
    ...
    hash_from_gateway = @gateway.send(:build_authorize_request, money, card_token, {})
  ...
  def test_build_credit_request_token
    ...
    hashFromGateway = @gateway.send(:build_credit_request, 1000, card_token, options)
    assert_equal nil, hashFromGateway['litleTxnId']
    assert_equal 'ecommerce', hashFromGateway['orderSource']
    assert_equal '1234', hashFromGateway['orderId']
    assert_equal '171299999999999', hashFromGateway['token']['litleToken']
  ...
  def test_purchase_pass_with_card_token
    purchaseResponseObj = {'response' => '000', 'message' => 'successful', 'litleTxnId'=>'123456789012345678'}
    retObj = {'response'=>'0','saleResponse'=>purchaseResponseObj}
    LitleOnline::Communications.expects(:http_post => retObj.to_xml(:root => 'litleOnlineResponse'))
    ...
    card_token = LitleCardToken.new(:token => '171299999999999',
                                    :month => '11',
                                    :year  => '2014')
    responseFrom = @gateway.purchase(0, card_token)
    assert_equal true, responseFrom.success?
    assert_equal '123456789012345678', responseFrom.authorization
  end
